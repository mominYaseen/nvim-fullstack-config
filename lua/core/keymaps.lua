...
-- for runnign springboot debug on 5005 port
vim.keymap.set("n", "<leader>js", function()
  vim.cmd("wa") -- Save all buffers

  -- Open a new terminal buffer in Neovim
  vim.cmd("split term://bash") -- splits window and opens terminal
  local term_buf = vim.api.nvim_get_current_buf()

  -- Send the Spring Boot command to the terminal
  vim.fn.chansend(vim.b.terminal_job_id,
    [[mvn spring-boot:run -Dspring-boot.run.jvmArguments="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005"]]
    .. "\n"
  )

  print("Spring Boot started in debug mode on port 5005. Logs are visible in this terminal split.")
end, { desc = "Run Spring Boot app in terminal with JDWP debug on 5005" })
